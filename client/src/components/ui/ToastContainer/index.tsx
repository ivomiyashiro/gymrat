...
export const ToastContainer = () => {
  ...
  return (
    <div className={ `fixed flex justify-center w-full px-4 transition-all duration-300 ${show ? 'opacity-100 bottom-6 ' : 'opacity-0 -bottom-96'}` }>
      {toast && (
        <div className={ `relative text-white px-6 py-4 pl-6 pr-16 rounded flex gap-10 shadow-xl ${ toast.type === 'SUCCESS' && 'bg-green-400' } ${ toast.type === 'WARNING' && 'bg-orange-400' } ${ toast.type === 'INFO' && 'bg-blue-400' } ${ toast.type === 'ERROR' && 'bg-red-400' }` }>
